...
toWrite = "Say : A " + results[0][len(results[0]) - 1] + ", il fait " + str(float(temperature[0].attributes['value'].value.encode('utf-8','ignore')) - 273.15) + ' °C, pour une humidité ambiante de ' + humidity[0].attributes['value'].value.encode('utf-8','ignore') + ' %. Deux mots pour décrire le temps ? "' + weather[0].attributes['value'].value.encode('utf-8','ignore') + '"...'
